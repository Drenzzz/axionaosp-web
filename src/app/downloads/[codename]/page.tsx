...
import { BuildCardClient } from "@/components/downloads/BuildCardClient";
import { DeviceImageClient } from "@/components/downloads/DeviceImageClient";
import { MaintainerCardClient } from "@/components/downloads/MaintainerCardClient";
    ...
    <div className="pt-36 pb-20 min-h-screen">
      <div className="container mx-auto px-4 max-w-4xl space-y-10">
        ...
        <header className="relative flex flex-col items-center justify-center text-center gap-4 py-8">
          <DeviceImageClient src={info.image_url} alt={info.device_name} className="w-auto h-60 md:h-72 object-contain drop-shadow-2xl" />
        ...
        <MaintainerCardClient maintainerName={info.maintainer} githubUsername={info.github_username} avatarUrl={avatarUrl} githubUrl={githubUrl} supportGroupUrl={info.support_group} />
        ...
        <div className="backdrop-blur-sm space-y-6">
          ...
          {!details.gms && !details.vanilla && <div className="text-center text-neutral-400 py-8 bg-neutral-800/50 rounded-lg border border-neutral-700">No official builds found for this device yet.</div>}
        </div>
              ...
              <Accordion type="single" collapsible className="w-full px-2">
                <AccordionItem value="item-1" className="border-b-0">
                  ...
                  <AccordionContent className="px-4 pb-4">
                    <pre className="text-sm bg-neutral-900/50 p-4 rounded-lg text-neutral-300 whitespace-pre-wrap font-sans max-h-80 overflow-y-auto custom-scrollbar border border-neutral-700">{details.changelog}</pre>
                  </AccordionContent>
                </AccordionItem>
              </Accordion>
            </Card>
